...
const TESTER_INVITE_FIELDS = "email,name,phone,tester_type,application_id" as const;
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: tester, error } = await supabaseAdmin.from("beta_testers").select(TESTER_INVITE_FIELDS).eq("id", id).maybeSingle();
