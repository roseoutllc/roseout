...
export default function AdminMessagingCenter() {
  ...
  async function sendMessage() {
    ...
    const deliveryNotes = [payload.channelResults?.email, payload.channelResults?.sms]
      .filter((value) => value && value !== "not requested")
      .join(" • ");

    setNotice({
      type: "success",
      text: `Message processed for ${payload.recipient?.name || "recipient"}.${
