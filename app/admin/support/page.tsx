...
export default async function AdminSupportPage() {
  ...
  const supportTickets = (tickets || []) as SupportTicket[];
        ...
        <section className="mt-6 overflow-hidden rounded-[2rem] border border-white/10 bg-[#111] shadow-2xl">
          ...
          <div className="divide-y divide-white/10">
            {supportTickets.map((ticket) => (
              <Link key={ticket.id} href={`/admin/support/${ticket.id}`} className="grid gap-3 p-5 transition hover:bg-white/[0.04] md:grid-cols-[1fr_170px_150px] md:items-center">
                <div>
