    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col p-3 sm:p-3.5">
        ...
        <div className="mt-auto space-y-2 border-t border-white/10 pt-3">
          ...
          <div className="grid min-h-[42px] grid-cols-2 gap-2">
            {websiteUrl ? (
              <a
                href={websiteUrl}
                target="_blank"
              ...
              <span aria-hidden="true" className="invisible rounded-full px-3 py-2.5 text-xs font-black">
                Website
              </span>
            )}
...
function getWhyPicked({
  ...
  const ratingCue = rating ? `${rating} rose rating` : "";
  const locationCue = distanceCue || (address ? "clear location details" : "");
  const supportCue = [locationCue, ratingCue].filter(Boolean).join(" • ");
