...
const DEFAULT_ADMIN_NOTIFY_EMAIL = "nick@roseout.com";
const DEFAULT_ADMIN_NOTIFY_PHONE = "+16467891953";
...
export function getAdminNotifyPhone() {
  return DEFAULT_ADMIN_NOTIFY_PHONE;
}
...
export function renderSupportEmail({
  ...
  const styles = {
    container: compact ? "max-width:640px;margin:0 auto;padding:20px 24px 28px;" : "max-width:920px;margin:0 auto;padding:24px 56px 44px;",
    title: compact ? "margin:0 0 20px;font-size:28px;line-height:1.15;font-weight:900;letter-spacing:-0.03em;color:#000000;" : "margin:0 0 150px;font-size:64px;line-height:1.05;font-weight:900;letter-spacing:-0.06em;color:#000000;",
    brand: compact ? "margin:0 0 24px;font-size:12px;line-height:1;font-weight:900;letter-spacing:0.14em;color:#e11d48;" : "margin:0 0 72px;font-size:28px;line-height:1;font-weight:900;letter-spacing:0.16em;color:#e11d48;",
    greeting: compact ? "margin:0 0 24px;font-size:16px;line-height:1.4;font-weight:700;color:#141414;" : "margin:0 0 150px;font-size:48px;line-height:1.2;font-weight:400;color:#141414;",
    body: compact ? "font-size:15px;line-height:1.6;font-weight:400;color:#141414;" : "font-size:48px;line-height:1.55;font-weight:400;color:#141414;letter-spacing:-0.035em;",
    ctaWrap: compact ? "margin:24px 0 0;" : "margin:56px 0 0;",
    cta: compact ? "display:inline-block;background:#e11d48;color:#ffffff;text-decoration:none;border-radius:999px;padding:11px 16px;font-size:14px;font-weight:900;letter-spacing:0.02em;" : "display:inline-block;background:#e11d48;color:#ffffff;text-decoration:none;border-radius:999px;padding:18px 28px;font-size:18px;font-weight:900;letter-spacing:0.02em;",
    signature: compact ? "margin:28px 0 0;" : "margin:150px 0 0;",
    rule: compact ? "border:0;border-top:1px solid #eeeeee;margin:24px 0 16px;" : "border:0;border-top:3px solid #eeeeee;margin:78px 0 44px;",
    footer: compact ? "margin:0;font-size:12px;line-height:1.4;color:#777777;" : "margin:0;font-size:36px;line-height:1.4;color:#777777;",
  };

  const cta = ctaUrl && ctaLabel
    ? `<p style="${styles.ctaWrap}"><a href="${ctaUrl}" style="${styles.cta}">${htmlEscape(ctaLabel)}</a></p>`
    : "";

  return `
    <div style="margin:0;padding:0;background:#ffffff;color:#141414;font-family:Arial,Helvetica,sans-serif;">
      <div style="${styles.container}">
        <h1 style="${styles.title}">${htmlEscape(title)}</h1>
        <p style="${styles.brand}">ROSEOUT</p>
        <p style="${styles.greeting}">${htmlEscape(greeting)}</p>
        <div style="${styles.body}">
          ${bodyHtml}
          ${cta}
          <p style="${styles.signature}">— RoseOut Team</p>
        </div>
        <hr style="${styles.rule}" />
        <p style="${styles.footer}">${htmlEscape(footerText)}</p>
      </div>
    </div>
  `;
}

function supportTicketActivityTime(ticket: SupportTicket) {
  return ticket.last_message_at || ticket.updated_at || ticket.created_at;
}

function newerSupportTicket(current: SupportTicket | undefined, next: SupportTicket) {
  if (!current) return next;

  const currentTime = new Date(supportTicketActivityTime(current)).getTime();
  const nextTime = new Date(supportTicketActivityTime(next)).getTime();
...
export async function listSupportTickets(limit = 50) {
  ...
  for (const ticket of [...fallbackTickets, ...primaryTickets]) {
    ticketsById.set(ticket.id, newerSupportTicket(ticketsById.get(ticket.id), ticket));
  }

  return Array.from(ticketsById.values())
    .sort((a, b) => {
      return new Date(supportTicketActivityTime(b)).getTime() - new Date(supportTicketActivityTime(a)).getTime();
    })
    .slice(0, limit);
}
...
export async function autoCloseInactiveSupportTickets({
  ...
  const tickets = await listSupportTickets(limit);
  const candidates = tickets.filter((ticket) => {
    if ((ticket.status || "open") === "closed") return false;

    return new Date(supportTicketActivityTime(ticket)).getTime() <= cutoff;
  });
  ...
  for (const ticket of candidates) {
    try {
      await closeSupportTicket(ticket.id, {
        reason: `Ticket auto-closed after ${days} days with no activity.`,
      });
      closedTicketIds.push(ticket.id);
    } catch (error) {
      errors.push(error instanceof Error ? error.message : `Could not close ${ticket.id}`);
    }
  }

  return {
    closed: closedTicketIds.length,
    closedTicketIds,
    errors,
  };
}
...
export async function closeSupportTicket(
  ticketId: string,
  { reason = "Ticket closed by RoseOut Support." }: { reason?: string } = {}
) {
...
async function notifySupportReply(ticket: SupportTicket, message: string, actorType: SupportActor) {
  ...
  await sendNotification({
    ...
    emailHtml: renderSupportEmail({
      ...
      size: creatorIsRecipient ? "default" : "compact",
      bodyHtml: `
